...
export default function CarouselSection({
    ...
    setAnimating(true);
    setIndex(i => i + delta);
    const t = setTimeout(() => setAnimating(false), TRANSITION_MS);
  ...
  // Si < 5 items, on duplique circulairement.
    ...
    if (n === 0) return [];
    const out: { article: Article; offset: -2 | -1 | 0 | 1 | 2; abs: number }[] = [];
    const targets = [(-2 as const), -1 as const, 0 as const, 1 as const, 2 as const];
    for (const off of targets) {
        const abs = index + off;                 // indice "absolu" (peut être négatif)
        const art = circularAt(base, abs);       // article correspondant modulo n
        out.push({ article: art, offset: off, abs });
    }
    return out;
    }, [base, n, index]);
			...
			<div className="inner">
				...
				<div className="carousel" role="region">
					...
					<div className="carousel-viewport" aria-live="polite">
						{visible.map(({ article, offset }, i) => (
							<a
								key={n >= 5 ? article.id : `${article.id}-${i}`}
								href={article.url}
								target="_blank"
								rel="noopener"
